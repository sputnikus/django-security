...
receiver = get_backend_receiver('elasticsearch')
...
def is_processing_celery_task(task_name, related_objects=None):
    CeleryTaskInvocationLog._index.refresh()
    qs = CeleryTaskInvocationLog.search().filter(
        Q('term', name=task_name)
        & Q('terms', state=[
            CeleryTaskInvocationLogState.ACTIVE.name,
            CeleryTaskInvocationLogState.WAITING.name,
            CeleryTaskInvocationLogState.TRIGGERED.name
        ])
    )
    related_object_keys = [
        get_key_from_object(related_object) for related_object in related_objects or ()
    ]

    for related_object_key in related_object_keys:
        qs = qs.filter(
